    ...
    for (byte i=0; i < len; i++) {
      ...
      if (lineaddr % 2) {
        ...
        if (b != (spi_transaction(0x28, lineaddr >> 9, lineaddr / 2, 0) & 0xFF)) {
          Serial.print("Erro no endereco 0x"); Serial.print(lineaddr, HEX);
          Serial.print(" devia ser 0x"); Serial.print(b, HEX); Serial.print(" e nao 0x");
